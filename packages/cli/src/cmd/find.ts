...
import { resolve as resolveThing } from "@omgbase/core";
...
function runFind(cli: Cli, args: string[]): number {
  ...
  if (values.help) {
    cli.io.out("  find <text> [-n N] [-1] [-v]  — ranked search; -1 prints the top id alone");
    return EXIT_OK;
  }
  ...
  const hits = resolveThing(ws.store, { repoId: repo.repoId, query: text, limit });
...
export const cmdFind: Command = { name: "find", summary: "Ranked search for the id of a thing", run: (cli, a) => runFind(cli, a) };
