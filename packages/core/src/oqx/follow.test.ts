...
// type-preserving relation. Sub-clauses (successor `where`, `frontier`, `depth`,
// `by`) live in a `{ … }` block; a bare `follow <relation>` carries none.
//
...
// recursion, so a single-root subtree walk is expressible.
...
describe("follow block.children — single-root subtree walk", () => {
  ...
  it("classifies $stop: interior vs leaf (relation ran out)", () => {
    ...
    expect(byItem.get("root")).toBe("interior");
    expect(byItem.get("mid")).toBe("interior");
    expect(byItem.get("sibling")).toBe("leaf");
    expect(byItem.get("leaf")).toBe("leaf");
  });
  ...
  it("`depth 2` bounds the walk; rows at the cap classify $stop == 'depth'", () => {
    ...
    expect(atCap.every((h) => h.s === "depth")).toBe(true);
...
describe("follow — consumers over a recursive query", () => {
  ...
  it("count folds the walk to a number (CTE binds first in the reduced branch)", () => {
    expect(run('repo.blocks count { where type == "list_item" && text.startsWith("root") follow block.children }').count).toBe(8);
  });

  it("exists answers presence over the walk", () => {
    expect(run('repo.blocks exists { where text.startsWith("root") follow block.children }').exists).toBe(true);
    expect(run('repo.blocks exists { where text == "nope" follow block.children }').exists).toBe(false);
  });
});
...
describe("follow — $ordinal (deterministic rank + budget)", () => {
  ...
  it("$ordinal is a contiguous 1..N rank ordered by (depth, path); the seed ranks first", () => {
    ...
    expect(ords).toEqual([1, 2, 3, 4, 5, 6, 7, 8]);
    expect(hits[0]!.d).toBe(1);
  });
