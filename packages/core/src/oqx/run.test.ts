...
describe("OQX end-to-end — relative source projection (`from E`)", () => {
  beforeEach(() => {
    ingest("canon/tasks.md", "---\nlayer: canon\n---\n\n# Work\n\n- [ ] ship oqx\n- [x] write spec\n");
    ingest("canon/prose.md", "---\nlayer: canon\n---\n\n# Prose\n\njust text, no tasks\n");
  });

  it("repo.docs collect { from nodes } yields the projected-through nodes (domain nodes)", () => {
    const { hits } = run('repo.docs collect { from nodes where kind == "md:task" select v: value }');
    expect(hits.map((h) => h.v).sort()).toEqual(["ship oqx", "write spec"]);
  });

  it("equals the direct nodes query for a collect consumer", () => {
    const via = run('repo.docs collect { from nodes where kind == "md:task" select v: value }').hits.map((h) => h.v).sort();
    const direct = run('from nodes where kind == "md:task" select v: value').hits.map((h) => h.v).sort();
    expect(via).toEqual(direct);
  });

  it("repo.nodes collect { from doc } yields the owning docs (bag, one per node)", () => {
    // canon/tasks.md has 3 nodes (section + 2 tasks); each projects to its doc,
    // so the doc appears once per node (bag semantics, not deduplicated).
    const { hits } = run("repo.nodes collect { from doc select p: $path }");
    const tasksCount = hits.filter((h) => h.p === "canon/tasks.md").length;
    expect(tasksCount).toBeGreaterThan(1);
    // every hit is a real doc path
    expect(hits.every((h) => h.p === "canon/tasks.md" || h.p === "canon/prose.md")).toBe(true);
  });
});

describe("OQX end-to-end — implicit leading clauses", () => {
  beforeEach(() => {
    ingest("canon/tasks.md", "---\nlayer: canon\n---\n\n# Work\n\n- [ ] ship oqx\n- [x] write spec\n");
    ingest("canon/prose.md", "---\nlayer: canon\n---\n\n# Prose\n\njust text, no tasks\n");
  });

  it("an implicit where filters identically to an explicit one", () => {
    const implicit = run('from docs where nodes exists { kind == "md:task" }').hits.map((h) => h.path);
    const explicit = run('from docs where nodes exists { where kind == "md:task" }').hits.map((h) => h.path);
    expect(implicit).toEqual(explicit);
    expect(implicit).toEqual(["canon/tasks.md"]);
  });

  it("a bare reference projects (implicit select), it does not filter", () => {
    const { hits } = run('from docs select tasks: nodes collect { value }');
    const byPath = new Map(hits.map((h) => [h.path, h.tasks as { value: string | null }[]]));
    // no filtering happened — EVERY node projects its `value` (section nodes → null).
    // prose.md keeps its lone section node; tasks.md keeps section + both tasks.
    expect(byPath.get("canon/prose.md")!.length).toBe(1); // just the section node
    const taskVals = byPath.get("canon/tasks.md")!.map((t) => t.value);
    expect(taskVals).toContain("ship oqx");
    expect(taskVals).toContain("write spec");
    expect(taskVals.length).toBe(3);
  });

  it("an implicit where narrows a top-level query", () => {
    const implicit = run('from docs layer == "canon"').hits.map((h) => h.path).sort();
    const explicit = run('from docs where layer == "canon"').hits.map((h) => h.path).sort();
    expect(implicit).toEqual(explicit);
  });
});
...
describe("OQX end-to-end — text() full-text predicate", () => {
  ...
  it("composes INSIDE a correlated subquery", () => {
...
describe("OQX end-to-end — top-level consumers", () => {
  beforeEach(() => {
    ingest("canon/a.md", "---\nlayer: canon\n---\n\n# A\n\n- [ ] t\n");
    ingest("canon/b.md", "---\nlayer: canon\n---\n\n# B\n\nprose\n");
    ingest("draft/c.md", "---\nlayer: draft\n---\n\n# C\n\nprose\n");
  ...
  it("repo.docs collect { … } is explicitly the same as the bare form", () => {
    const bare = run('from docs where layer == "canon"');
    const wrapped = run('repo.docs collect { where layer == "canon" }');
    expect(wrapped.consumer).toBe("collect");
    expect(wrapped.hits.map((h) => h.path)).toEqual(bare.hits.map((h) => h.path));
  });

  it("repo.docs count { … } reduces the query to the matching-row count (no hits)", () => {
    const r = run('repo.docs count { where layer == "canon" }');
  ...
  it("count over a correlated where counts docs, not their nodes", () => {
    const r = run('repo.docs count { where nodes exists { where kind == "md:task" } }');
    expect(r.count).toBe(1);
  });

  it("repo.docs exists { … } is a boolean over the outer row set", () => {
    expect(run('repo.docs exists { where layer == "draft" }').exists).toBe(true);
    expect(run('repo.docs exists { where layer == "ghost" }').exists).toBe(false);
  });

  it("repo.docs first { … } returns exactly the first row in path order (with its projections)", () => {
    const r = run('repo.docs first { where layer == "canon" select l: layer }');
  ...
  it("repo.docs first { … } over an empty match is zero rows, not an error", () => {
    expect(run('repo.docs first { where layer == "ghost" }').hits).toEqual([]);
  });

  it("repo.docs single { … } returns the one matching row", () => {
    const r = run('repo.docs single { where layer == "draft" }');
    expect(r.consumer).toBe("single");
    expect(r.hits.map((h) => h.path)).toEqual(["draft/c.md"]);
  });

  it("repo.docs single { … } fails loudly when more than one row matches", () => {
    expect(() => run('repo.docs single { where layer == "canon" }')).toThrow(/matched more than one row/);
  });

  it("repo.docs single { … } over no match is zero rows", () => {
    expect(run('repo.docs single { where layer == "ghost" }').hits).toEqual([]);
  });

  it("consumers wrap a full query — where + a nested collect projection survive", () => {
    const r = run('repo.docs first { where nodes exists { where kind == "md:task" } select tasks: nodes collect { where kind == "md:task" select v: value } }');
    expect(r.hits[0]!.path).toBe("canon/a.md");
    expect((r.hits[0]!.tasks as { v: string }[]).map((t) => t.v)).toEqual(["t"]);
  });

  it("rejects an unknown top-level consumer", () => {
    expect(() => run("repo.docs frobnicate { }")).toThrow(/unknown consumer/);
  });

  it("reserves `all` but reports it unimplemented", () => {
    expect(() => run("repo.docs all { }")).toThrow(/not implemented yet/);
  });

  it("rejects a consumer block with no closing brace", () => {
    expect(() => run("repo.docs count { where layer == \"canon\"")).toThrow(FilterInvalid);
  });

  it("a top-level consumer and the repo.<target> ROOT RELATION coexist (same sigil, different roles)", () => {
    const r = run('repo.docs first { where layer == "draft" select canon: repo.docs collect { where layer == "canon" select p: $path } }');
    expect(r.consumer).toBe("first");
...
describe("OQX end-to-end — boolean composition over consumer directives", () => {
  beforeEach(() => {
    ingest("tasks.md", "---\nlayer: draft\n---\n\n# T\n\n- [ ] work\n");
    ingest("links.md", "---\nlayer: draft\n---\n\n# L\n\nSee [x](/x.md).\n");
    ingest("plain.md", "---\nlayer: draft\n---\n\n# P\n\njust prose\n");
...
describe("OQX end-to-end — order by", () => {
  ...
  it("repo.docs first honors the order (the top-ranked row)", () => {
    expect(run("repo.docs first { order by ord desc }").hits.map((h) => h.path)).toEqual(["c.md"]);
  });
});
