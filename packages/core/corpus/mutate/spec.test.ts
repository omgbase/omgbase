// The mutation spec conformance runner: every case under spec/mutate/cases is a
// mutation script (spec/store §9.4's observation-script shape plus the `apply` /
// `macro` / `docs` / `plan` / `disk` steps) run by the reference against a
// `:memory:` store and an in-memory doc store; the step outcomes, the doc store's
// files and the store projection are compared to the committed `expect`. The
// fixture contract is spec/mutate/README.md §9.
//
// MUTATE_SPEC_UPDATE=1 rewrites each case's `expect` in place from its inputs
// (inputs, notes and case order untouched) before running; the diff is
// reviewed like code, because a changed `expect` *is* a mutation change.
import { describe, it, expect } from "vitest";
import { existsSync, readFileSync, readdirSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import { fileURLToPath } from "node:url";
import { deepEqualTol, runCase, validateFixtureFile } from "./fixture.js";
import type { FixtureCase, FixtureFile } from "./fixture.js";

// packages/core/corpus/mutate → repo root is four levels up.
const SPEC_DIR = fileURLToPath(new URL("../../../../spec/mutate/", import.meta.url));
const CASES_DIR = join(SPEC_DIR, "cases");

const UPDATE = (() => {
  const v = process.env.MUTATE_SPEC_UPDATE;
...
function regenerate(): string[] {
  ...
  for (const file of fileNames()) {
    ...
    for (const c of doc.cases) {
      const { expect: generated, problems } = runCase(c);
...
if (UPDATE) {
  ...
  process.stderr.write(`[mutate spec] regenerated expectations under ${CASES_DIR}\n${report.join("\n")}\n`);
}
...
function run(c: FixtureCase): void {
  const { expect: actual, problems: invariantProblems } = runCase(c);
...
describe("mutate spec fixtures (spec/mutate/cases)", () => {
  it("spec/mutate/VERSION is <major>.<minor> (README 'Versioning')", () => {
