// The properties spec conformance runner: every case under spec/properties/cases
// is ingested by the reference into a `:memory:` store, its `properties` rows
// read back, checked against the §7 runner checks, and — with the two read
// shapes of §5 — compared to the committed `expect`. The fixture contract is
// spec/properties/README.md §7.
//
// PROPERTIES_SPEC_UPDATE=1 rewrites each case's `expect` in place from its
// `source` (inputs, notes and case order untouched) before running; the diff is
// reviewed like code, because a changed `expect` *is* a properties change.
...
// packages/core/corpus/properties → repo root is four levels up.
const SPEC_DIR = fileURLToPath(new URL("../../../../spec/properties/", import.meta.url));
const CASES_DIR = join(SPEC_DIR, "cases");

const UPDATE = (() => {
  const v = process.env.PROPERTIES_SPEC_UPDATE;
  return v !== undefined && v !== "" && v !== "0";
})();

/** Every number here is a stored double that JSON round-trips exactly; compare exactly. */
const EPS = 0;
...
/** Rewrite every case's `expect` from its `source`; returns the change report. */
...
if (UPDATE) {
  ...
  process.stderr.write(`[properties spec] regenerated expectations under ${CASES_DIR}\n${report.join("\n")}\n`);
}
...
describe("properties spec fixtures (spec/properties/cases)", () => {
  it("spec/properties/VERSION is <major>.<minor> (README 'Versioning')", () => {
