// The reconcile spec conformance runner: every case under spec/reconcile/cases
// is run through the reference matcher, checked against the §2/§3 invariants,
// and its canonicalized result compared to the committed `expect`. The fixture
// contract is spec/reconcile/README.md §9.
//
// RECONCILE_SPEC_UPDATE=1 rewrites each case's `expect` in place from its
// inputs (inputs, notes and case order untouched) before running; the diff is
// reviewed like code, because a changed `expect` *is* a matcher change.
import { describe, it, expect } from "vitest";
import { existsSync, readFileSync, readdirSync, writeFileSync } from "node:fs";
import { join } from "node:path";
import { fileURLToPath } from "node:url";
import { deepEqualTol, evaluateCase, validateFixtureFile } from "./fixture.js";
import type { FixtureCase, FixtureFile } from "./fixture.js";

// packages/core/corpus/reconcile → repo root is four levels up.
const CASES_DIR = fileURLToPath(new URL("../../../../spec/reconcile/cases/", import.meta.url));

const UPDATE = (() => {
  const v = process.env.RECONCILE_SPEC_UPDATE;
  return v !== undefined && v !== "" && v !== "0";
})();

/** Numbers are IEEE doubles carried at full precision; compare within 1e-9 (README §8). */
const EPS = 1e-9;
...
if (UPDATE) {
  ...
  process.stderr.write(`[reconcile spec] regenerated expectations under ${CASES_DIR}\n${report.join("\n")}\n`);
}
...
function runCase(c: FixtureCase): void {
  const { expect: actual, problems: invariantProblems } = evaluateCase(c);
...
describe("reconcile spec fixtures (spec/reconcile/cases)", () => {
