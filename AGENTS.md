...
- **Rust side** (cargo workspace at the root `Cargo.toml` over `crates/*`, independent of the pnpm workspace). The Rust port proceeds component by component, each behind a language-neutral spec under `spec/` that both implementations run: **fixture first, TypeScript (reference) second, Rust third**. A behavior change without a fixture is not done.
  ...
  - `spec/graph` — **the graph specification**: `README.md` (node kinds and `node_id`, extraction x2 as exact rules, resolution, interval maintenance, rollup, phantom adoption, URI normalization, fixture contract, oddities) + `VERSION` + `cases/*.json` (observation scripts, `GRAPH_SPEC_UPDATE=1` on `packages/core/corpus/graph/spec.test.ts`). 
  - `crates/omgbase-search` — the Rust search layer: the FTS query sanitizer, embedding inputs (context prefix, cache keys, document input, whole-vs-pooled budget, pooling), cosine, RRF fusion and boosts, the stdio/HTTP provider protocol and the fixture embedder; `omgbase-store` runs the queries and the drain (store 13.3). Runner runs `spec/search/cases`.
  - `spec/search` — **the search specification**: `README.md` (FTS5 index maintenance and sanitizer, `text_search`, embeddings — what embeds, inputs, cache, document input, pooling — cosine, vector search, fusion and boosts, `resolve`, the provider protocol, the fixture embedder, fixture contract, oddities) + `VERSION` + `cases/*.json` (`SEARCH_SPEC_UPDATE=1` on `packages/core/corpus/search/spec.test.ts`). 
  - `crates/omgbase-mutate` — the Rust mutation kernel: the working tree, the six ops (insert/update/move/remove/split/merge) with placement addressing and CAS, the splice renderer's dirty rules, changeset placeholder resolution, the whole-document lowering and the opset (pure); `omgbase-store` loads, applies, commits (`api` ingest with known ids), runs macros and document operations (store 13.4). Runner runs `spec/mutate/cases`.
  - `spec/mutate` — **the mutation specification**: `README.md` (working tree, the six ops as exact rules, rendering, changesets and the commit protocol, macros, document operations, the whole-document planner, errors, fixture contract, oddities) + `VERSION` + `cases/*.json` (`MUTATE_SPEC_UPDATE=1` on `packages/core/corpus/mutate/spec.test.ts`). Remaining after this: the sync registry; then MCP/CLI.
