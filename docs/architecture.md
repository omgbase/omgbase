...
- The commit's `origin` determines the legal history claim:
  ...
  - `import` is reserved for bulk ingest with freshly minted ids and explicitly no retro-inferred block history; nothing in-tree produces it today (the mrplex importer is gone), so every intent write is `api` and every sync ingest is `observed`.
