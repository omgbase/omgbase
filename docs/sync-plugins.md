...
Source resolution now **prefers the registry**: `omg watch` / `omg mcp` call `openRepoSource(store, repo)` (`cli/src/cmd/_source.ts`), which finds the repo's attached `fs` source and spawns `@omgbase/fs-adapter` (bin **`omgbase-fs-adapter`**) with `renderConfigFlags(config)` (`{ root }` → `--root <root>`). When a repo has **no** registered source it **falls back to synthesizing the fs source from `root_path`** — the pre-registry behavior — so existing single-fs repos are unchanged until `attach` populates the registry (Stage 5) and `root_path` is retired (Stage 6). A repo with neither a registered source nor a `root_path` is **sourceless** and its `sync`/`watch` is a no-op (§7). (`omg sync`'s freshness fast-path still reads `root_path` directly; unifying it onto the registry + the single `observeFile` reconcile primitive is Stage 4 / ADR-014 D2.)
