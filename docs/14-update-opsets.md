...
- **update `{ trivia }`** — trailing-trivia tiling (`retiled`) set exactly for inserted/moved blocks and any block whose trivia changed, so the committed bytes equal the proposed content precisely.
...
- **Small-block matcher policy.** A 1–2 token block (a terse paragraph, a short heading, a short list item) whose text changes is conservatively *not* carried by the reconciler (high similarity threshold) → its id resets. This is matcher policy, surfaced honestly in the plan, not a lowering limitation.
